...
class EDUDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # update NA aspect and polarity
        # pdb.set_trace()
        # asp_labels[self.asp_dict['na']] = 1
        # pol_labels[self.asp_dict['na']] = 1 # neutal

        # assert sum([1 if p == 1 else 0 for p in asp_labels]) == (num_asp + 1)
...
class DatasetLoader(object):
    def _tuple_batch_edu(self, batch_data):
        ...
        segs2idx, text2idx, _, segs, num_seg, num_asp, _, _, asp_labels, pol_labels = zip(*sorted_batch)
        ...
        num_word_sent = torch.LongTensor([len(r) for r in text2idx])
        batch_sent = torch.zeros(len(text2idx), num_word_sent.max().item()).long()
        for s_idx, sent2idx in enumerate(text2idx):
            batch_sent[s_idx, 0: len(sent2idx)] = torch.LongTensor(sent2idx)
        ...
        return batch_seg, segs, num_word_seg, num_seg, batch_sent, num_word_sent, sent_order, batch_asplabels, num_asp, batch_pol_labels

    def __init__(self, batch_size, asp_dict, input_data):
        _dataset = EDUDataset(input_data, asp_dict)
        self.num_batch = len(_dataset) // batch_size
        print(self.num_batch)
        self.Data = DataLoader(_dataset, batch_size=batch_size, shuffle=True,  num_workers=0, collate_fn=self._tuple_batch_edu, pin_memory=True)
